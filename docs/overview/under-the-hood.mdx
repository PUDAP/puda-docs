...
Validation checks that the protocol shape is correct and that each command's `params` match the machine catalog's [JSON primitive](../setup/integrating-a-new-machine#json-primitives) types before anything reaches a physical machine. If validation fails, the agent fixes the protocol and validates again.
