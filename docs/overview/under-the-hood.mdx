...
2. The agent reads the installed [PUDA skills](https://github.com/pudap/skills), chooses the `puda-protocol` skill, to understand how to craft a valid protocol.
...
Validation checks that the protocol shape is correct and that each named command's `params` match the live catalog's [JSON primitive](../setup/integrating-a-new-machine#json-primitives) types before anything reaches a physical machine. Only commands in the protocol are parsed; unused catalog entries are ignored. Unannotated parameters, `*args`, and `**kwargs` skip the type check. If validation fails, the agent fixes the protocol and validates again.
...
Optionally, a separate [PUDA logger](../observability/logging-service) can be configured to store everything on a central [InfluxDB](https://www.influxdata.com/) database.
