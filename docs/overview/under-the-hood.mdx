...
To run commands on different machines in parallel, say so in natural language. The agent uses the `puda-protocol` skill to generate a protocol where those commands share the same `step_number`.
