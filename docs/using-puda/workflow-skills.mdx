...
- **Failure handling** — what to do when a command fails, including when to stop and report.
- **Example runs** — one or more complete command sequences the agent can follow.

## Include failure handling

Workflow skills should describe both the normal path and the safe stop path. If a command fails, the agent should preserve machine state, report what happened, and avoid guessing the next physical action.
...
- If aspiration or dispense fails, do not retry automatically. Report the failure and leave the robot in its current state.
...
- The safe approach path to well B3 requires raising Z to at least 50 mm before any XY move.
- Position queries can lag by up to 1 second after a move, so wait briefly before verifying position.
- If the machine reports BUSY, wait and query state again instead of issuing another command.
