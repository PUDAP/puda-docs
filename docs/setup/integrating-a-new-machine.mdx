...
The telemetry handler should publish at least heartbeat, position, health, and state. Position should describe the current physical coordinates. State should describe what the machine is doing and include the active `run_id` if the hardware or edge service tracks one.
...
async def telemetry_handler():
    ...
    await edge_nats_client.publish_position({
        "x_mm": driver.position.x_mm,
        "y_mm": driver.position.y_mm,
        "z_mm": driver.position.z_mm,
    })

    await edge_nats_client.publish_health({
        "connected": driver.is_connected(),
        "error": driver.last_error,
