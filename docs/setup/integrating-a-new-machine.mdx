...
Each subdirectory is a standalone edge service from the [puda-python-edge](https://github.com/PUDAP/puda-python-edge) template. Give each one a unique `MACHINE_ID` in its `.env` file, then start them individually or with a small script that launches every edge process on the PC.
