...
- Annotate `@command` parameters with [JSON primitives](#json-primitives) only. Protocol validate rejects types that cannot be represented in JSON.
...
`puda protocol validate` checks each command's `params` against the live catalog. Parameter types must be **JSON primitives** because protocol files and command payloads are JSON, so the catalog can only describe types JSON can carry.
