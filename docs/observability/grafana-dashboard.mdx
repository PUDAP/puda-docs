...
Once the [PUDA Logger](./logging-service) is writing to InfluxDB, you can add Grafana for dashboards, exploration, and alerts. Grafana queries the same `machine_status` and `machine_commands` measurements the logger writes from NATS.
...
Edit `.env` and point `INFLUXDB_URL` at the InfluxDB server from the [PUDA Logger setup](./logging-service#start-influxdb). Use the same token and database names you configured for the PUDA Logger and Hermes Logger:
