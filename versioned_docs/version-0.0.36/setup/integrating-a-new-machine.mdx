...
Use the [edge-python-template](https://github.com/PUDAP/edge-python-template/tree/1450ca5f8e8f8f627d204954856553fadf23edfd) template to create a new repository. Name it `puda-<machine_id>-edge`, replacing `<machine_id>` with your machine ID. The template depends on the `puda` Python SDK from PyPI. Pin it to **0.0.16** for this CLI release:

```bash
pip install puda==0.0.16
```
...
Each subdirectory is a standalone edge service from the [edge-python-template](https://github.com/PUDAP/edge-python-template/tree/1450ca5f8e8f8f627d204954856553fadf23edfd) template. Give each one a unique `MACHINE_ID` in its `.env` file, then start them individually or with a small script that launches every edge process on the PC.
...
All public methods on the driver class are exposed as machine commands on PUDA, allowing an AI agent to access them using the PUDA CLI. This CLI release requires Python SDK **0.0.16**.
...
Prefer atomic commands that map to a single hardware action. Avoid commands that combine unrelated work, hide required inputs, or require complex objects that cannot be represented cleanly in the [protocol file](https://docs.puda.co/docs/overview/under-the-hood#protocol-generation).
